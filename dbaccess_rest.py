...
class JsonDatabase:
    ...
    def __create_collection(self):
        ...
        if self.__collname in all_colls:
            raise DatabaseCollectionExists("{} already exists".format(self.__collname))
        resp = requests.put(url=self.__baseurl, auth=(self.__user, self.__pword))
    ...
    def add_item(self, item_dict):
        ...
        """
        resp = self.__make_post_request(target_url=self.__baseurl, payload_dict=item_dict)
        if resp['status'] == "error":
            error_msg = str(resp.get("code")) + " " + resp.get("reason")
    ...
    def add_metadata(self, metadata_dict):
        ...
        resp = self.__make_post_request(target_url=self.__baseurl, payload_dict=formatted_dict)
        if resp['status'] == "error":
            error_msg = str(resp.get("code")) + " " + resp.get("reason")
            raise DatabaseUnableToAddItem(message=error_msg)
        else:
    ...
    def extract_metadata(self):
        ...
        if resp['status'] == 'ok':
            ...
            if count > 1:
                error_msg = "Found {} metadata documents. Only one is allowed.".format(count)
    ...
    def add_multiple_items(self, items_list):
        ...
        resp = self.__make_post_request(target_url=insert_url, payload_dict=items_list)
        if resp['status'] == "error":
            error_msg = str(resp.get("code")) + " " + resp.get("reason")
            raise DatabaseUnableToAddItem(message=error_msg)
        else:
    ...
    def extract_items(self, z_layer):
        ...
        if resp['status'] == 'ok':
            ...
            if has_more:
                print("Database reports the return of only a fraction of all items. ")
                print("This method has not been designed yet for very large data retrievals.")
                sys.exit(0)
    ...
    def __make_delete_request(self, target_url):
        resp = requests.delete(url=target_url, auth=(self.__user, self.__pword))
